...
void WiimotedevConnection::cwiid_process_nunchuk_stick(uint8 cwiid_stick[2]) {
  ...
  if(stick[ix_nunchuk_stick].x > nunchukStickMaxX) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_STICK_RIGHT;
  else if(stick[ix_nunchuk_stick].x < nunchukStickMinX) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_STICK_LEFT;

  if(stick[ix_nunchuk_stick].y > nunchukStickMaxY) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_STICK_UP;
  else if(stick[ix_nunchuk_stick].y < nunchukStickMinY) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_STICK_DOWN;
}
...
void WiimotedevConnection::cwiid_process_nunchuk_acc(uint8 cwiid_acc[3]) {
  ...
  if(acc[ix_nunchuk_device].pitch < -45.0) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_TILT_FRONT;
  else if(acc[ix_nunchuk_device].pitch >  45.0) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_TILT_BACK;

  if(acc[ix_nunchuk_device].roll >  45.0) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_TILT_RIGHT;
  else if(acc[ix_nunchuk_device].roll < -45.0) cstate[ix_nunchuk_device] |= NUNCHUK_BTN_TILT_LEFT;
  ...
  if(ncounter == 0 && (nfXmotion.count() > 0 || nfYmotion.count() > 0 || nfZmotion.count() > 0)) {
    ...
    if(nxvalueabs > 20) nyvalueabs -= 80;
    else if(nxvalueabs > 10) nyvalueabs -= 40;
    else if(nxvalueabs > 5) nyvalueabs -= 20;

    if(nyvalue < -4) nyvalueabs += 60;
    else if(nyvalue < -3) nyvalueabs += 40;
    else if(nyvalue < -2) nyvalueabs += 20;
    else if(nyvalue < -1) nyvalueabs += 10;
