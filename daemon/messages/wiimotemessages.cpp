...
void WiimotedevConnection::cwiid_process_wiimote_acc(uint8 cwiid_acc[3]) {
  ...
  if (acc[ix_wiimote_device].pitch < -45.0) cstate[ix_wiimote_device] |= WIIMOTE_BTN_TILT_FRONT;
  else if (acc[ix_wiimote_device].pitch > 45.0) cstate[ix_wiimote_device] |= WIIMOTE_BTN_TILT_BACK;

  if (acc[ix_wiimote_device].roll > 45.0) cstate[ix_wiimote_device] |= WIIMOTE_BTN_TILT_RIGHT;
  else if (acc[ix_wiimote_device].roll < -45.0) cstate[ix_wiimote_device] |= WIIMOTE_BTN_TILT_LEFT;
  ...
  if ((wcounter == 0) && ((wfXmotion.count() > 0) || (wfYmotion.count() > 0) || (wfZmotion.count() > 0))) {
    ...
    if (wxvalueabs > 20) wyvalueabs -= 80;
    else if (wxvalueabs > 10) wyvalueabs -= 40;
    else if (wxvalueabs > 5) wyvalueabs -= 20;

    if (wyvalue < -4) wyvalueabs += 60;
    else if (wyvalue < -3) wyvalueabs += 40;
    else if (wyvalue < -2) wyvalueabs += 20;
    else if (wyvalue < -1) wyvalueabs += 10;
