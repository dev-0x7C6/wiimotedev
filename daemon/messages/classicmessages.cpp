...
void WiimotedevConnection::cwiid_process_classic_init() {
  cwiid_process_classic_clear();
}

void WiimotedevConnection::cwiid_process_classic_done() {
  cwiid_process_classic_clear();
}
...
void WiimotedevConnection::cwiid_process_classic_lstick(uint8 cwiid_stick[2]) {
  ...
  if (stick[ix_classic_lstick].x > classicLStickMaxX) cstate[ix_classic_device] |= CLASSIC_BTN_LSTICK_RIGHT;
  else if (stick[ix_classic_lstick].x < classicLStickMinX) cstate[ix_classic_device] |= CLASSIC_BTN_LSTICK_LEFT;

  if (stick[ix_classic_lstick].y > classicLStickMaxY) cstate[ix_classic_device] |= CLASSIC_BTN_LSTICK_UP;
  else if (stick[ix_classic_lstick].y < classicLStickMinY) cstate[ix_classic_device] |= CLASSIC_BTN_LSTICK_DOWN;
}

void WiimotedevConnection::cwiid_process_classic_rstick(uint8 cwiid_stick[2]) {
  ...
  if (stick[ix_classic_rstick].x > classicRStickMaxX) cstate[ix_classic_device] |= CLASSIC_BTN_RSTICK_RIGHT;
  else if (stick[ix_classic_rstick].x < classicRStickMinX) cstate[ix_classic_device] |= CLASSIC_BTN_RSTICK_LEFT;

  if (stick[ix_classic_rstick].y > classicRStickMaxY) cstate[ix_classic_device] |= CLASSIC_BTN_RSTICK_UP;
  else if (stick[ix_classic_rstick].y < classicRStickMinY) cstate[ix_classic_device] |= CLASSIC_BTN_RSTICK_DOWN;
}

void WiimotedevConnection::cwiid_process_classic_buttons(uint16 cwiid_buttons) {
